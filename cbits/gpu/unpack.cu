...
__device__ inline auto unpack_one(uint64_t bits, int32_t const count,
                                  float* const out, int32_t const stride) noexcept -> void
{
...
__global__ auto unpack_kernel_cuda(SpinsInfo spins, OutInfo out) -> void
{
    ...
    for (auto i = idx; i < out.shape[0]; i += stride) {
        unpack_one(spins.data[i * spins.stride], out.shape[1], out.data + i * out.stride[0], out.stride[1]);
    }
}
} // namespace detail

auto unpack_cuda(torch::Tensor spins, int64_t const number_spins, torch::Tensor out) -> void
{
