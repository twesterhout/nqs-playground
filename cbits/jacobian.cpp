...
auto get_number_parameters(torch::autograd::variable_list const& parameters) -> int64_t
{
...
TCM_FORCEINLINE auto make_task(torch::Tensor const& in, Forward forward,
                               ...
                               torch::Tensor const& out) -> std::function<auto(int64_t)->void>
{
	...
	return [in, fn = std::move(forward), &parameters, out](auto const i) mutable {
		auto gradients = torch::autograd::grad(
			/*outputs=*/{fn(in[i])}, /*inputs=*/parameters,
...
TCM_EXPORT auto jacobian(torch::jit::script::Module const& module, torch::Tensor in,
                         std::optional<torch::Tensor> out, int num_threads) -> torch::Tensor
{
    using std::begin, std::end;
    ...
    if (!out.has_value()) {
		...
		auto const number_parameters = detail::get_number_parameters(parameters);
        out.emplace(torch::empty({batch_size, number_parameters},
			torch::TensorOptions{}.device(in.device()).dtype(dtype)));
    }
	auto task = detail::make_task(in,
