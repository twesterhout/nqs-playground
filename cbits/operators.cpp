...
TCM_FORCEINLINE auto _unpack(SpinBasis::StateT x, unsigned const number_spins,
...
TCM_EXPORT auto unpack(torch::Tensor x, int64_t const number_spins)
    -> torch::Tensor
{
    TCM_CHECK(
    ...
    auto const first = static_cast<uint64_t const*>(x.data_ptr());
    auto const last  = first + shape[0];
    ...
    torch::RegisterOperators{}
        .op("tcm::unpack",
            torch::RegisterOperators::options()
                .kernel<auto(torch::Tensor, int64_t)->torch::Tensor,
        ...
        .op("tcm::unpack_index",
            torch::RegisterOperators::options()
                .kernel<
                    auto(torch::Tensor, torch::Tensor, int64_t)->torch::Tensor,
                    static_cast<auto (*)(torch::Tensor, torch::Tensor, int64_t)
                                    ->torch::Tensor>(&unpack)>(
                    c10::TensorTypeId::CPUTensorId));
