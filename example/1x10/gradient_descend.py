...
def main():
    ...
    options = nqs_playground.sgd.Config(
        model=[make_amplitude("nade" if use_autoregressive else "dense"), make_phase()],
        output="1x10.result",
        exact=ground_state[:, 0],
        hamiltonian=operator,
        epochs=300,
        sampling_options=nqs_playground.SamplingOptions(number_samples=20000, number_chains=1),
        sampling_mode="autoregressive" if use_autoregressive else "exact",
        optimiser=lambda p: torch.optim.Adam(p, lr=1e-2),
    )
