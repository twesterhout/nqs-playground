...
# sys.path.append("/vol/tcm01/westerhout_tom/nqs-playground/external/SdLBFGS/optim")
...
def load_dataset(config):
    tqdm.write("Loading the basis...")
    basis = core.with_file_like(config.basis, "rb", pickle.load)
    tqdm.write("Building a list of representatives")
    basis.build()
    tqdm.write("Loading the ground state")
    ground_state = np.load(config.dataset)
    ground_state.shape = [-1, 1]
    return basis, (basis.states, eliminate_phase(ground_state))
